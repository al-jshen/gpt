...
class DTN(nn.Module):
    def __init__(
        ...
        if num_patches[0] > 7:  # can also set to 14
            self.patch_pool_size = num_patches[0] // patch_size
            self.patch_size = patch_size
        else:
            self.patch_pool_size = 1
            self.patch_size = num_patches[0]
        ...
        if not only_var:
            self.mean_norm_weight = nn.Parameter(torch.zeros(num_heads))
        self.var_norm_weight = nn.Parameter(torch.zeros(num_heads))
    ...
    def forward(self, input):
        ...
        if (
            not hasattr(self, "rel_indices")
            or self.rel_indices.size(1) != self.patch_size * self.patch_size
        ):
        ...
        assert T == H * W, "input feature {} has wrong size {}".format(
            input.shape, self.num_patches
        )
        ...
        if self.patch_pool_size > 1:
            mean_rppn = mean_rppn.view(
                B, self.num_heads, C // self.num_heads, self.patch_size, self.patch_size
            )
            mean_rppn = mean_rppn.unsqueeze(-1).repeat(
                1, 1, 1, 1, 1, self.patch_pool_size * self.patch_pool_size
            )
            mean_rppn = mean_rppn.view(
                B,
                self.num_heads,
        ...
        if not self.only_var:
            mean_rppn = (
                1.0 - mean_norm_weight.view(1, self.num_heads, 1, 1)
            ) * mean_rppn
            mean_rppn += mean_norm_weight.view(1, self.num_heads, 1, 1) * mean_ln.view(
                B, 1, 1, -1
            )
            mean_rppn = mean_rppn.view(B, C, T).permute(0, 2, 1)

        var_rppn = (1.0 - var_norm_weight.view(1, self.num_heads, 1, 1)) * var_rppn
        var_rppn += var_norm_weight.view(1, self.num_heads, 1, 1) * var_ln.view(
            B, 1, 1, -1
        )
...
if __name__ == "__main__":
    dtn = DTN(num_features=384, num_heads=8)
    x = torch.rand(2, int(16 * 16), 384)
    y = dtn(x)
